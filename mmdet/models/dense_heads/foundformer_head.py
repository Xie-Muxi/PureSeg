...
class FoundFormerHead(MaskFormerHead):
    """Implements the Found2Former head.

    See `Masked-attention Mask Transformer for Universal Image
    Segmentation <https://arxiv.org/pdf/xxxx>`_ for details.
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.spectral_extractors = nn.ModuleList(
            [SpectralFeatureExtractor(in_ch, in_ch) for in_ch in in_channels]
        )
    ...
    def forward(
        ...
        # 提取和融合光谱特征
        spectral_features = [
            extractor(torch.flip(feat, [3]))
            for extractor, feat in zip(self.spectral_extractors, x)
        ]
        combined_features = [
            torch.cat([orig_feat, spec_feat], dim=1)
            for orig_feat, spec_feat in zip(x, spectral_features)
        ]

        # mask_features, multi_scale_memorys = self.pixel_decoder(x)
        # ? 使用 combined_features 替换原始的 x 进行后续处理
        mask_features, multi_scale_memorys = self.pixel_decoder(combined_features)
        ...
        # 翻转输入图像并提取光谱特征
        flipped_x = [torch.flip(feat, [3]) for feat in x]  # 假设 x 是一个特征图列表
        spectral_features = [
            self.spectral_feature_extractor(feat) for feat in flipped_x
        ]

        # 特征融合（需确保尺寸一致）
        combined_features = [
            torch.cat([original, spectral], dim=1)
            for original, spectral in zip(x, spectral_features)
        ]
