...
from rsseg.registry import MODELS
...
class SAMAggregatorNeck(nn.Module):
    ...
    def forward(self, inputs):
        print('--------------------------')
        print(f'Length of inputs: {len(inputs)}') # Length of inputs: 1
        print(f'Type of inputs: {type(inputs)}') # Type of inputs: <class 'tuple'>
        # _, _, _, inner_states = inputs
        # print(type(inputs[-1]))

        
        # _, inner_states = inputs
        # inner_states = inputs[0]

        # print(f'Length of inner_states: {len(inner_states)}')
        # print(f'Type of inner_states: {type(inner_states)}')
        # print(f'Shape of inner_states: {inner_states.shape}')
        # inner_states = inner_states.unsqueeze(0)
        # inner_states = [einops.rearrange(inner_states[idx], 'b h w c -> b c h w') for idx in self.selected_channels]
        # inner_states = [layer(x) for layer, x in zip(self.down_sample_layers, inner_states)]

    def forward(self, inputs):
        inner_states = inputs[0].unsqueeze(0)
        ...
        return tuple([img_feats_2, img_feats_1, img_feats_0])